...
module SecureVote.Contracts.FaucetErc20 where
...
approve :: forall e. TransactionOptions NoPay -> { _spender :: Address, _amount :: (UIntN (D2 :& D5 :& DOne D6)) } -> Web3 e HexString
approve x0 r = uncurryFields  r $ approve' x0
   where
    approve' :: TransactionOptions NoPay -> Tagged (SProxy "_spender") Address -> Tagged (SProxy "_amount") (UIntN (D2 :& D5 :& DOne D6)) -> Web3 e HexString
...
transferFrom :: forall e. TransactionOptions NoPay -> { _from :: Address, _to :: Address, _amount :: (UIntN (D2 :& D5 :& DOne D6)) } -> Web3 e HexString
transferFrom x0 r = uncurryFields  r $ transferFrom' x0
   where
    transferFrom' :: TransactionOptions NoPay -> Tagged (SProxy "_from") Address -> Tagged (SProxy "_to") Address -> Tagged (SProxy "_amount") (UIntN (D2 :& D5 :& DOne D6)) -> Web3 e HexString
...
transfer :: forall e. TransactionOptions NoPay -> { _to :: Address, _amount :: (UIntN (D2 :& D5 :& DOne D6)) } -> Web3 e HexString
transfer x0 r = uncurryFields  r $ transfer' x0
   where
    transfer' :: TransactionOptions NoPay -> Tagged (SProxy "_to") Address -> Tagged (SProxy "_amount") (UIntN (D2 :& D5 :& DOne D6)) -> Web3 e HexString
...
allowance x0 cm r = uncurryFields  r $ allowance' x0 cm
   where
    allowance' :: TransactionOptions NoPay -> ChainCursor -> Tagged (SProxy "_owner") Address -> Tagged (SProxy "_spender") Address -> Web3 e (Either CallError (UIntN (D2 :& D5 :& DOne D6)))
...
type FaucetFn = Tagged (SProxy "faucet()") (Tuple0 )

faucet :: forall e. TransactionOptions NoPay -> Web3 e HexString
faucet x0 = sendTx x0 ((tagged $ Tuple0 ) :: FaucetFn)
